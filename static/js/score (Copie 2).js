...
function getScore(instrument, nb_pages) {
    ...
    for (var i = 1; i <= nb_pages; i++) {
        ...
        // On attend que la première page soit chargée pour calculer le layout
    loadedPages[0].onload = function() {
        var nb_showed_pages = countDisplayablePages(container, loadedPages[0]);
        console.log("on peut afficher", nb_showed_pages, "page(s)");

        // On ne crée l'affichage qu'une fois qu'on connaît le nombre de pages
